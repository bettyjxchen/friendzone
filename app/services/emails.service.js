...
function _formRegistrationEmailBody(email) {
	let url = `www.friendzone.app/${email.id}`;

	let body = {
		text: `You have a message from: ${email.senderName} (${email.senderEmail})
        Message:
        ${email.message}
...
function _sendEmail(toEmail, senderEmail, subject, text, html) {
	...
	return sgMail.send(email).catch(err => console.log(err.toString()));
}
