...
function readAll() {
    return conn.conn().collection('hackers').find().toArray()
        .then( hackers => {
...
function readById(id) {
    return conn.conn().collection('hackers').findOne({ _id: new ObjectId(id) })
        .then(hacker => {
            hacker._id = hacker._id.toString() // convert ObjectId back to string
            return hacker
        })
...
function create(model) {
    return conn.conn().collection('hackers').insert(model)
...
function update(id, doc) {
    ...
    return conn.conn().collection('hackers').replaceOne( { _id: new ObjectId(id) }, doc )
        .then(result => Promise.resolve()) // "return" nothing
}

function _delete(id) {
    return conn.conn().collection('hackers').deleteOne({ _id: new ObjectId(id) })
        .then(result => Promise.resolve()) // "return" nothing
}
